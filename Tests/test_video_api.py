...
recall_data['recalls'].sort(key = lambda x: datetime.strptime(x['ReportReceivedDate'], '%d/%m/%Y'), reverse=True)

print(recall_data['recalls'])
info = video_api.get_car_videos(car, recall_data)

print(info)

# display_recalls(info)